...
const ProductSchema = ({ productId }) => {
  ...
  useEffect(() => {
    ...
    const fetchProductAndReviews = async () => {
      try {
        ...
        const foundProduct = productData.items.find(item => item.itemId === productId);
        setProduct(foundProduct?.item || null);

        // Fetch reviews
        const reviewsRes = await fetch(`https://api.malidag.com/get-reviews/${productId}`);
        const reviewsData = await reviewsRes.json();
        if (reviewsData.success) {
          setReviews(reviewsData.reviews);

          const total = reviewsData.reviews.reduce((sum, r) => sum + (parseFloat(r.rating) || 5), 0);
  ...
  return (
     <Helmet>
      ...
      <meta property="og:description" content={`Buy ${product.name} on Malidag. ${product.text || 'Discover great personal care products'}`} />
      <meta property="og:image" content={product.images?.[0]} />
      <meta name="twitter:card" content="summary_large_image" />
      <meta name="twitter:title" content={`${product.name} | Malidag`} />
      <meta name="twitter:description" content={`Buy ${product.name} on Malidag — ${product.theme || 'best in beauty and personal care'}`} />
